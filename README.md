...
```text
使用 $book-grilling 预制《书名》下一单元，完成后停下。
```

如果工作区只有一本正在学习的书，可以省略书名。技能会自动找到课程和下一
单元，完成原文提取、问题树生成与独立复核，然后写入旁路缓存；不要求用户
提供单元 ID、revision、页码、来源范围或 JSON 参数。

当前单元结束时，学习会话会自动校验并接管缓存，直接进入下一单元根问题。
如果后台预制刚好晚于单元边界完成，下一次“继续”会自动激活。损坏、过期或
不完整的缓存不会阻塞学习，也不会改写进度或提前泄露答案。

缓存始终只预制紧接着的一个单元，并按书籍、来源和课程结构指纹隔离，因此
同一套机制可以安全用于不同作品。
...
已经使用 Book Grilling 1.0 创建的 `.book-grilling/course.json` 无需迁移，
可以直接使用 1.1.0；预制缓存是可选旁路文件，不会重写既有学习进度。
...
来源证据篡改、错误分支归档、目录顺序、学习单元范围重叠、跨会话预制、
自动接管、边界竞态、缓存损坏降级以及 1.0 课程只读兼容。
